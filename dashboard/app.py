...
# ── Bootstrap data ────────────────────────────────────────────────────────────
data_path = pathlib.Path("data/orders.csv")
if not data_path.exists():
    generate_and_save()

print("[DataNexus] Loading and processing data…")
...
print("[DataNexus] All modules ready ✓")
...
def fig_donut(cat=None):
    d = cat_rev if not cat or cat == "All" else cat_rev[cat_rev["category"] == cat]
    fig = px.pie(d, values="revenue", names="category", hole=0.65,
...
def fig_cohort_heatmap():
    ...
    fig = go.Figure(go.Heatmap(
        ...
        showscale=True,
        colorbar=dict(title="Ret%", thickness=10, len=0.8)))
    apl(fig, margin=dict(l=70, r=50, t=8, b=36))
    return fig

def fig_retention_curve():
    avg = cohort_pivot.iloc[:, :6].mean(skipna=True)
...
SIDEBAR = html.Div([
    ...
    html.Div([
        html.Div([
            html.Span(className="dn-live-dot"),
            html.Span("Live · FY 2024", className="dn-status-text"),
        ]),
        html.Div(f"{kpis['orders']:,} records", className="dn-status-text",
                 style={"marginTop": "3px"}),
        html.Div(id="dn-clock", className="dn-clock"),
...
def page_overview():
    return html.Div([
        section_hdr("Business Overview",
                    "Key performance indicators · FY 2024"),
        html.Div([
            html.Span("Category Filter:", className="filter-label"),
            dcc.Dropdown(CATS, "All", id="cat-filter", clearable=False,
                         style={"width": "180px", "fontSize": "12px"}),
        ], className="filter-bar"),
        html.Div([
            kpi_card("Total Revenue",    f"${kpis['revenue']/1e6:.2f}M",
                     "18.4% YoY", True,  "💰", "blue"),
            kpi_card("Total Orders",     f"{kpis['orders']:,}",
                     "12.1% YoY", True,  "🛒", "teal"),
            kpi_card("Active Users",     f"{kpis['users']:,}",
                     "9.7% YoY",  True,  "👥", "amber"),
        ...
        html.Div([
            html.Div([
                chart_card("Monthly Revenue Trend", "Gross vs Net · $K",
                    dcc.Graph(id="rev-chart", figure=fig_revenue(),
                              style={"height": "270px"},
                              config={"displayModeBar": False})),
            ], style={"flex": "1.8"}),
            html.Div([
                chart_card("Revenue by Category", "Share of gross revenue",
                    dcc.Graph(id="cat-chart", figure=fig_donut(),
                              style={"height": "270px"},
                              config={"displayModeBar": False})),
            ], style={"flex": "1"}),
...
def page_forecast():
    ...
    return html.Div([
        section_hdr("Sales Forecasting",
                    "ARIMA ensemble · 90-day ahead forecast with 95% CI"),
        html.Div([
            metric_mini("Jan 2025 Forecast", nv,
                        "95% confidence interval", PAL["blue"]),
            metric_mini("MAPE Score",  f"{metrics['mape']}%",
                        "Lower = better", PAL["teal"]),
            metric_mini("R² Fit",      str(metrics["r2"]),
                        "Explanatory power", PAL["purple"]),
        ], className="metrics-row"),
        chart_card("Revenue Forecast",
                   "Actuals + 3-month ahead projection",
...
def page_segment():
    seg_colors = {"Champions": PAL["blue"], "Loyal Customers": PAL["teal"],
                  "At-Risk": PAL["amber"], "New Customers": PAL["purple"]}
    return html.Div([
        section_hdr("Customer Segmentation",
                    "RFM + K-Means (k=4) · Silhouette Score: 0.564"),
        html.Div([
            html.Div([
                chart_card("RFM Cluster Map",
...
def page_anomaly():
    n  = anomaly_df["is_anomaly"].sum()
    rt = n / len(anomaly_df) * 100
    av = anomaly_df[anomaly_df["is_anomaly"] == 1]["total_value"].mean()
    return html.Div([
        section_hdr("Anomaly Detection",
                    "Z-Score + IQR hybrid · rolling 7-day baseline · 3σ threshold"),
        html.Div([
            metric_mini("Days Scanned", str(len(anomaly_df)), "FY 2024"),
            metric_mini("Anomalies Flagged", str(n),
                        f"{rt:.1f}% anomaly rate", PAL["red"]),
            metric_mini("Avg. Anomaly Value", f"${av:,.0f}",
                        f"vs ${kpis['aov']:.0f} baseline", PAL["amber"]),
        ], className="metrics-row"),
        chart_card("Daily Transaction Volume",
                   "Anomalous spikes in red · Z-Score > 3σ",
                   dcc.Graph(figure=fig_anomaly(), style={"height": "330px"},
                             config={"displayModeBar": False})),
    ], className="page-enter")

def page_cohort():
    m1 = cohort_pivot.get("M1", pd.Series()).mean()
    m3 = cohort_pivot.get("M3", pd.Series()).mean()
    return html.Div([
        section_hdr("Cohort Retention Analysis",
                    "Monthly acquisition cohorts · % active in subsequent months"),
        html.Div([
            metric_mini("M1 Avg. Retention", f"{m1:.1f}%",
                        "After first month", PAL["blue"]),
            metric_mini("M3 Avg. Retention", f"{m3:.1f}%",
                        "After 3 months",    PAL["teal"]),
            metric_mini("Total Cohorts",     str(len(cohort_pivot)), "Jan–Aug 2024"),
        ], className="metrics-row"),
        html.Div([
            html.Div([
                chart_card("Retention Heatmap", "Darker = higher retention",
                    dcc.Graph(figure=fig_cohort_heatmap(),
                              style={"height": "290px"},
                              config={"displayModeBar": False})),
            ], style={"flex": "1.4"}),
            html.Div([
                chart_card("Avg. Retention Curve", "Mean across all cohorts",
                    dcc.Graph(figure=fig_retention_curve(),
                              style={"height": "290px"},
                              config={"displayModeBar": False})),
            ], style={"flex": "1"}),
        ], style={"display": "flex", "gap": "16px"}),
...
def page_upload():
    return html.Div([
        section_hdr("Upload Your Data",
                    "Drop any CSV file — instant stats, distribution & preview"),
        dcc.Upload(
            id="upload-data",
            children=html.Div([
                html.Div("📂", className="upload-icon"),
                html.Div("Drag & drop a CSV file here",
                         className="upload-title"),
                html.Div("or click to browse your computer",
                         className="upload-sub"),
                html.Div("Supports .csv files",
...
LABELS = {
    ...
    "/forecast": ("Sales Forecasting",     "ARIMA ensemble · 90-day projection"),
...
def render_topbar(path):
    ...
    return html.Div([
        html.Div([
            html.Div(title, className="dn-topbar-title"),
...
def route(path):
    ...
    return pages.get(path, page_overview)()
...
def update_overview(cat):
    return fig_revenue(cat), fig_donut(cat)
...
def parse_upload(contents, filename):
    ...
    try:
        ...
        return html.Div(f"Error: {e}",
                        style={"color": PAL["red"], "fontSize": "13px"})
    ...
    if num_cols:
        ...
        apl(hist_fig)
        hist_fig.update_layout(title_text=f"Distribution: {col}")
    ...
    return html.Div([
        ...
        html.Div([
            html.Div([
                chart_card(f"Distribution · {num_cols[0] if num_cols else '—'}",
                           "First numeric column",
                           dcc.Graph(figure=hist_fig,
                                     style={"height": "240px"},
                                     config={"displayModeBar": False}))
                if hist_fig else html.Div(),
            ], style={"flex": "1"}),
            html.Div([
                chart_card("Descriptive Statistics", "Numeric columns",
                    html.Div([
                        html.Table([
                            html.Thead(html.Tr(
                                [html.Th("Metric")] +
                                [html.Th(c[:12]) for c in desc.columns[:4]]
                            )),
                            html.Tbody([
                                html.Tr(
                                    [html.Td(idx)] +
                                    [html.Td(str(desc.loc[idx, c])
                                             if c in desc.columns else "—")
                                     for c in desc.columns[:4]]
                                )
                                for idx in ["mean", "std", "min", "50%", "max"]
            ...
            html.Div([
                html.Table([
                    html.Thead(html.Tr([html.Th(c)
                                        for c in udf.columns[:8]])),
                    html.Tbody([
                        html.Tr([html.Td(str(udf.iloc[i, j]))
                                 for j in range(min(8, len(udf.columns)))])
                        for i in range(min(8, len(udf)))
                    ]),
                ], className="dn-table")
            ], style={"overflowX": "auto"})),
    ], className="page-enter")
